...
const BadgePanel: React.FC = () => {
  ...
  const updateBadgeScale = (delta: number) => {
    const newScale = Math.max(0.5, Math.min(2, badge.scale + delta));
    updateField('badge', { ...badge, scale: newScale });
  };
    ...
    <Accordion
      ...
      icon={
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          <path d="M2 4l6 8 6-8H2z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
        </svg>
      }
    >
          ...
          {BUILTIN_BADGES.map((b) => (
            <button
              ...
              style={{
                ...
                fontSize: '0.8rem',
                fontWeight: badge.builtinId === b.id ? 600 : 400,
                cursor: 'pointer',
      ...
      <div className="form-group">
        <label className="form-group__label">角标大小</label>
        <div className="form-number">
          <button className="form-number__btn" onClick={() => updateBadgeScale(-0.1)}>-</button>
          <span className="form-number__value">{Math.round(badge.scale * 100)}%</span>
          <button className="form-number__btn" onClick={() => updateBadgeScale(0.1)}>+</button>
        </div>
      </div>
      ...
      {(badge.builtinId || badge.customImage) && (
        <div style={{ marginTop: 12 }}>
          <Button variant="danger" size="sm" onClick={clearBadge}>
            清除角标
          </Button>
        </div>
      )}
